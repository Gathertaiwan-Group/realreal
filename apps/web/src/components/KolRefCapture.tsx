...
import { resolveKolRef } from "@/lib/kol-ref"

/**
 * Client-side KOL referral capture (spec I).
 *
 * Mounted once in the root layout. On every navigation, captures the KOL slug
 * from either `?ref=<slug>` (any page) or the `/k/<slug>` landing path, and
 * persists it as a 7-day cookie so the checkout API can attribute the order to
 * the right KOL even if the customer browses around before buying.
 *
...
export function KolRefCapture() {
  ...
  useEffect(() => {
    const ref = resolveKolRef(pathname, searchParams.get("ref"))
    if (!ref) return
